...
MODEL_SAVE_NAME = "{}model.yml"
CONFIG_SAVE_NAME = "{}config.yml"
...
def save(model, config, model_dir):
    ...
    model_path = MODEL_SAVE_NAME.format(model_dir)
    config_path = CONFIG_SAVE_NAME.format(model_dir)
    
    if(os.path.exists(model_dir)):
        print("the base path exsits: "+model_dir)
    else:
        print("not found the path: "+model_dir)
    
    utils.check_mandatory_keys(config, ["name", "optimizer", "loss", "x", "y"])
    with open(config_path, "w") as f:
        yaml.dump(config, f, default_flow_style=False)
        print(config_path+": successfully open")
    with open(model_path, "w") as f:
        f.write(model.to_yaml())
        print(model_path+": successfully open")
